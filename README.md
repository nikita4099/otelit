...
ответ с номерами айди которые на её взгляд приведут к сделке. Шаг 1. Убрать всё не подходящее. Шаг 2. Ранжировать подходящее. Шаг 3. Дать вводные по цене и вероятности. 
...
- На закрытие каких БТ подписываемся в данной итерации `Data Scientist`: всё
...
| Название данных  | Есть ли данные в компании (если да, название источника/витрин) | Требуемый ресурс для получения данных (какие роли нужны) | Проверено ли качество данных (да, нет) |
| ------------- | ------------- | ------------- | ------------- |
| Менеджер | Айди  | DE | + | - айди пользователя используем не как число, а как класс. Модел должна учитывать, что у разных менеджеров разное представление подходит/не подходит.
| Компания/лид.Чем занимается | Чем занимается | DE | - |
| Компания/лид.Площади от | Площади от | DE | - |
| Компания/лид.Площади до | Площади до | DE | - |
| Компания/лид.По какому объекту звонили | Воозможно, список надо сопоставить со списком объектов. А может и нет | DE | - |
| Товар свободный.Площадь  | Площадь  | DE | + |
| Список категорий, присутствующих на объекте  | для каждого товара берём айди его объекта, по этому айди смотрим все товары. В товарах смотрим действующего арендатора и в этих действующий арендаторах берём поле "Чем занимается", собираем это всё в единый список  | DE | + |
| Товар свободный.Цена за всё помещение  | Цена за всё помещение  | DE | + |
...
- Какие метрики качества используем и почему они связаны с бизнес-результатом, обозначенным `Product Owner` в разделах `1` и `3`. Пример - WAPE <= 50% для > 80% категорий, bias ~ 0. Возможна формулировка в терминах относительно бейзлайна, количественно. Для бейзлайна могут быть свои целевые метрики, а может их вообще не быть (если это обосновано) `Data Scientist`: прокси метрики = конверсия из лида в сделку, скорость обработки лидов.
- Необходимый результат этапа. Например, необходимым результатом может быть не просто достижение каких-либо метрик качества, а включение в модели определенных факторов (флаг промо для прогноза выручки, др.) `Data Scientist`: приемлимость данных, определена модель.
- Какие могут быть риски и что планируем с этим делать. Например, необходимый для модели фактор (флаг промо) окажется незначимым для большинства моделей. Или для 50% моделей будет недостаточно данных для оценки `Data Scientist`: данные хреново заполнены - будем привлекать практикатов для разметки данных. 
...
- Предусмотрена ли бизнес-проверка результата этапа и как будет проводиться `Data Scientist` & `Product Owner` : проверка Дмитрием.
  
### 3. Подготовка пилота  
  
#### 3.1. Способ оценки пилота  
  
- Краткое описание предполагаемого дизайна и способа оценки пилота `Product Owner`, `Data Scientist` with `AB Group`: вместо "подбора" у менеджера появляется всплывающее окно. В нём по умолчанию показывается доска свободных помещений + поле для наполнение товарами, рекомендованными системой. При открытии окна данные о клиенте отправляются в модель. Модель отправляет айди товаров подходящих клиенту, в интерфесе эти айди открываются в заданном формате. Успешным пиломо будет 
  
#### 3.2. Что считаем успешным пилотом  
  
Формализованные в пилоте метрики оценки успешности `Product Owner`: accuracy = 0.9
...
- Блок схема и пояснения: сервисы, назначения, методы API `Data Scientist` : попробуем на Docker сделать. ML как сервис. Через API запросы идут.
  
#### 4.2. Описание инфраструктуры и масштабируемости 
  
- Какая инфраструктура выбрана и почему `Data Scientist`: надо изучить варианты
...
- Потенциальная уязвимость системы `Data Scientist`  : тут могу сказать, что всё может не заработать из-за того, что данные очень растянуты по времени и в идеале обучение делать не на старых данных, а на срезе данных. Какие были помещения в момент заключения сделки кроме сданного помещения.
...
- Описание взаимодействия между сервисами (методы API и др.) `Data Scientist` : пока хз
  
#### 4.6. Риски  
  
- Описание рисков и неопределенностей, которые стоит предусмотреть `Data Scientist` : пока хз
